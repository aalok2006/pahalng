...
use PahalNGO\VolunteerSubmission;

// --- Configuration ---
define('RECIPIENT_EMAIL_CONTACT', "pahalasadi@gmail.com");
define('RECIPIENT_EMAIL_VOLUNTEER', "pahalasadi@gmail.com");
define('SENDER_EMAIL_DEFAULT', 'pahalasadi@gmail.com');
define('SENDER_NAME_DEFAULT', 'PAHAL NGO Website');
...
// validate_data function removed - using Laravel Validator
...
// get_db_connection function removed - using Eloquent Capsule
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($submitted_form_id === 'contact_form') {
        ...
        // Laravel Validation
        $validator = validator($form_submissions[$form_id], [
            'name' => 'required|string|min:2|max:100', // alpha_space requires custom rule or regex, using string for simplicity or regex
        ...
        if ($validator->fails()) {
            ...
            // Flatten generic Laravel errors to match existing simple structure (field => first_msg)
            foreach ($validation_errors as $field => $msgs) { $form_errors[$form_id][$field] = $msgs[0]; }
        }

        if (empty($form_errors[$form_id])) {
            ...
            if (send_email($to, $subject, $body, $email, $name, $logContext, $logFile)) {
                try {
                    ...
                    log_message("{$logContext} Data saved to database via Eloquent. Name: {$name}", $logFile);
                    $form_messages[$form_id] = ['type' => 'success', 'text' => "Thank you, {$name}! Your message has been sent successfully."]; $form_submissions[$form_id] = [];
                } catch (\Exception $e) { // Catch generic Exception for Eloquent
        ...
        } else { $errorCount = count($form_errors[$form_id]); $form_messages[$form_id] = ['type' => 'error', 'text' => "Please correct the " . ($errorCount > 1 ? $errorCount . " errors" : "error") . " below."]; log_message("{$logContext} Validation failed.", LOG_FILE_ERROR); }
    ...
    elseif ($submitted_form_id === 'volunteer_form') {
        ...
        // Laravel Validation
        $validator = validator($form_submissions[$form_id], [
            'volunteer_name' => 'required|string|min:2|max:100',
            'volunteer_email' => 'required_without:volunteer_phone|nullable|email|max:255',
            'volunteer_phone' => 'required_without:volunteer_email|nullable|string|max:20',
            'volunteer_area' => 'required|in:Health Camps,Blood Donation Drives,Education Support,Environmental Projects,Skill Development,Event Management,General Support',
            'volunteer_availability' => 'required|string|min:5|max:150',
            'volunteer_message' => 'nullable|string|max:1000',
        ]);
            ...
            if (send_email($to, $subject, $body, $volunteer_email, $volunteer_name, $logContext, $logFile)) {
                try {
                    ...
                    VolunteerSubmission::create([
                        'name' => $volunteer_name,
                        'email' => $volunteer_email ?: null,
                        'phone' => $volunteer_phone ?: null,
                        'area_of_interest' => $volunteer_area,
                        'availability' => $volunteer_availability,
                        'message' => $volunteer_message ?: null,
                        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? null,
                        'submitted_at' => date('Y-m-d H:i:s')
                    ]);

                    log_message("{$logContext} Data saved to database via Eloquent. Name: {$volunteer_name}", $logFile);
                ...
                } catch (\Exception $e) { log_message("{$logContext} Database Insertion Error: " . $e->getMessage(), LOG_FILE_ERROR); $form_messages[$form_id] = ['type' => 'success', 'text' => "Thank you, {$volunteer_name}! Your application was sent, but there was an issue saving it to our records. We will still get back to you."]; $form_submissions[$form_id] = []; }
...
<head>
    ...
    <meta property="og:url" content="https://your-pahal-domain.com/">
    <meta property="og:image" content="https://your-pahal-domain.com/pahal_ngo_cover.jpg">
    <meta property="og:site_name" content="PAHAL NGO Jalandhar">
    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:url" content="https://your-pahal-domain.com/">
    <meta name="twitter:title" content="<?= htmlspecialchars($page_title) ?>">
    <meta name="twitter:description" content="<?= htmlspecialchars($page_description) ?>">
    <meta name="twitter:image" content="https://your-pahal-domain.com/pahal_ngo_cover.jpg">
    ...
    <script type="application/ld+json">
    {
      ...
      "contactPoint": [ { "@type": "ContactPoint", "telephone": "+91-9855614230", "contactType": "General Inquiry", "name": "Mr Bipan Suman" }, { "@type": "ContactPoint", "email": "<?= RECIPIENT_EMAIL_CONTACT ?>", "contactType": "General Inquiry" } ],
                ...
                <div class="contact-info animate-on-scroll fade-in-left">
                    ...
                    <div>
                        ...
                        <div class="contact-info-item">
                            ...
                            <div>
                                <h4>Phone:</h4>
                                <p><a href="tel:+919855614230">+91 9855614230</a></p>
                            </div>
                        </div>
                        <div class="contact-info-item">
        ...
        <div class="footer-grid">
            ...
            <div class="footer-col">
                ...
                <address>
                    ...
                    <p><i class="fas fa-phone-alt"></i> <a href="tel:+919855614230" class="footer-link">+91 9855614230</a></p>
