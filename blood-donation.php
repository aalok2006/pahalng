...
// --- Configuration ---
// Email Settings
define('RECIPIENT_EMAIL_DONOR_REG', "themohitrubel@gmail.com");
define('RECIPIENT_EMAIL_BLOOD_REQUEST', "themohitrubel@gmail.com");

define('SENDER_EMAIL_DEFAULT', 'themohitrubel@gmail.com');
define('SENDER_NAME_DEFAULT', 'PAHAL NGO Blood Program');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($submitted_form_id === 'donor_registration_form') {
        ...
        // Laravel Validation
        $validator = validator($submitted_data, [
            'donor_name' => 'required|string|min:2|max:100',
            'donor_email' => 'required|email|max:255',
            'donor_phone' => 'required|string|max:20', // Regex validation is flexible in Laravel or use custom
            'donor_blood_group' => 'required|in:'.implode(',', $blood_types),
            'donor_dob' => 'required|date_format:Y-m-d',
            'donor_location' => 'required|string|max:150',
        ]);
        
        $validation_errors = [];
        if ($validator->fails()) {
            $e = $validator->errors()->toArray();
            foreach ($e as $field => $msgs) { $validation_errors[$field] = $msgs[0]; }
        }
    ...
    elseif ($submitted_form_id === 'blood_request_form') {
        ...
        // Laravel Validation
        $validator = validator($submitted_data, [
            'request_patient_name' => 'required|string|min:2|max:100',
            'request_blood_group' => 'required|in:'.implode(',', $blood_types),
            'request_units' => 'required|integer|min:1|max:20',
            'request_hospital' => 'required|string|max:200',
            'request_contact_person' => 'required|string|min:2|max:100',
            'request_contact_phone' => 'required|string|max:20',
        ...
        // After validation, cast units to integer if validation passed (Laravel val checks integer but returns string)
...
<head>
    ...
    <meta property="og:url" content="https://your-pahal-domain.com/blood-donation.php"/>
    <meta property="og:image" content="https://your-pahal-domain.com/images/pahal-blood-og.jpg"/>
         ...
         <section id="contact-info" class="section-padding bg-neutral-light"> <!-- E-Waste section background -->
             <div class="container mx-auto text-center max-w-3xl">
                ...
                <div class="contact-block inline-block text-left space-y-4 max-w-md mx-auto animate-slide-up"> <!-- Simple div with spacing, animation -->
                    ...
                    <p><i class="fas fa-phone"></i> <strong>Direct Line:</strong> <a href="tel:+919041995969">+91 9041995969</a></p>
                    <p><i class="fas fa-envelope"></i> <strong>Email:</strong> <a href="mailto:themohitrubel@gmail.com?subject=Blood%20Donation%20Inquiry" class="break-all">themohitrubel@gmail.com</a></p> 
                </div>
